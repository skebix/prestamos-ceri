...
class Estadisticas extends CI_Controller {
    ...
    public function ver(){
        ...
        $data['title'] = 'Estad&iacute;sticas de '. $mes_title . ' de '. $ano . '.';
        ...
        $solicitudes = $this->estadisticas_model->get_solicitudes_by_year_and_month($ano, $mes);
        ...
        for($x = 0; $x < $N_categorias; $x++){
            ...
            while($n < $N_solicitudes){ //Recorrido del numero de solicitudes
                ...
                while($m < $N_usuarios){ //Recorrido del numero de usuarios
                    if(($solicitudes[$n]['id_solicitante'] == $usuarios[$m]['id']) and ($usuarios[$m]['id_categoria_usuario'] == $categorias[$x]['id'])){
        ...
        $sols_espacios = $this->estadisticas_model->get_solicitudes_espacios_usos();
        ...
        for($x = 0; $x < $N_espacios; $x++){ // inicio de la exploracion por espacio
            ...
            while($n < $N_solicitudes){
                ...
                while($m < $N_sols_espacios){
                    if(($solicitudes[$n]['id'] == $sols_espacios[$m]['id_solicitud']) and ($sols_espacios[$m]['id_espacio'] == $espacios[$x]['id'])){
                        $a = new DateTime($solicitudes[$n]['hora_entrega']);
                        $b = new DateTime($solicitudes[$n]['hora_devolucion']);
                        if($a == $b and $b == '0:00'){
                            $time = '0:00';
        ...
        $total3 = array("cantidad_total" => 0, "tiempo_total" => '0:00');

        $usos = $this->usos_model->get_usos('usos');
        $usos_espacios = $this->estadisticas_model->get_solicitudes_espacios_usos();
        ...
        for($x = 0; $x < $N_usos; $x++){ // inicio de la exploracion por espacio
            ...
            while($n < $N_solicitudes){
                ...
                while($m < $N_usos_espacios) {
                    if(($solicitudes[$n]['id'] == $usos_espacios[$m]['id_solicitud']) and ($usos_espacios[$m]['id_uso'] == $usos[$x]['id'])){
                        $a = new DateTime($solicitudes[$n]['hora_entrega']);
                        $b = new DateTime($solicitudes[$n]['hora_devolucion']);
                        if($a == $b and $b == '0:00'){
                            $time = '0:00';
        ...
        for($x = 0; $x < $N_equipos; $x++){ // inicio de la exploracion por espacio
            ...
            while($n < $N_solicitudes){
                ...
                while($m < $N_sols_equipos){
                    if(($solicitudes[$n]['id'] == $sols_equipos[$m]['id_solicitud']) and ($sols_equipos[$m]['id_equipo'] == $equipos[$x]['id'])){
                        $a = new DateTime($solicitudes[$n]['hora_entrega']);
                        $b = new DateTime($solicitudes[$n]['hora_devolucion']);
                        if($a == $b and $b == '0:00'){
                            $time = '0:00';
