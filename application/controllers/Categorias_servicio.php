    ...
    function crear(){
        ...
        if($administrador){
            ...
            if (!$this->form_validation->run()) {
                ...
                if($was_inserted){
                    $this->session->set_flashdata('success', 'Categor&iacute;a de servicio creada satisfactoriamente.');
                    redirect('categorias-servicio/listar');
                }

                //Si llegué a este punto es porque no pudo guardar el servicio
                $this->session->set_flashdata('danger', 'No se pudo crear su categor&iacute;a de servicio. Por favor intente nuevamente.');
                redirect('categorias-servicio/listar');
            }
        }else{
    ...
    public function actualizar($id){
        ...
        if($administrador){
            ...
            if($categoria){
                ...
                $this->session->set_flashdata('danger', 'La categoría que intenta actualizar no existe, o hubo un problema al conectarse con la Base de Datos. Por favor intente nuevamente.');
                redirect('inicio');
            }
            ...
            if (!$this->form_validation->run()){
                ...
                //Si lo guardó correctamente, redirigir al inicio con éxito
                if($was_updated){
                    $this->session->set_flashdata('success', 'Categor&iacute;a de servicio actualizada satisfactoriamente.');
                    redirect('categorias-servicio/listar');
                }

                //Si llegué a este punto es porque no pudo guardar el servicio
                $this->session->set_flashdata('danger', 'No se pudo actualizar su categor&iacute;a de servicio. Por favor intente nuevamente.');
                redirect('categorias-servicio/listar');
            }
        }else{
            //Si llegué a este punto es porque no ha ingresado, o no es Administrador
    ...
    public function eliminar($id){
        ...
        if($administrador){
            ...
            if($cantidad_servicios > 0){
                $this->session->set_flashdata('warning', 'Esta categor&iacute;a no puede ser eliminada, est&aacute; siendo utilizada por ' . $cantidad_servicios . ' servicios. Elimine los servicios primero, o deshabilite la categor&iacute;a en lugar de eliminarla.');
                redirect('categorias-servicio/listar');
            }else{
                ...
                if($categoria_servicio){
                    ...
                    if($delete_id){
                        $this->session->set_flashdata('success', 'Categor&iacute;a de servicio eliminada satisfactoriamente.');
                        redirect('categorias-servicio/listar');
                    }else{
                        $this->session->set_flashdata('danger', 'No se pudo eliminar su categor&iacute;a de servicio, por favor intente nuevamente');
                        redirect('categorias-servicio/listar');
                    }
                }else{
                    $this->session->set_flashdata('warning', 'La categor&iacute; de servicio que intenta eliminar no existe.');
                    redirect('categorias-servicio/listar');
                }
            }
        }else{
            ...
            if($categoria_servicio){
                if($categoria_servicio['habilitado']){
                    ...
                    if($was_updated){
                        ...
                        $this->session->set_flashdata('success', 'La categor&iacute;a de servicio fue deshabilitada satisfactoriamente. Recuerde que al deshabilitar una categor&iacute;a, tambi&eacute;n est&aacute; deshabilitando los servicios pertenencientes a la misma.');
                        redirect('categorias-servicio/listar');
                    }else{
                        $this->session->set_flashdata('danger', 'No se pudo deshabilitar la categor&iacute; de servicio, por favor intente nuevamente.');
                        redirect('categorias-servicio/listar');
                    }
                }else{
                    $this->session->set_flashdata('warning', 'La categor&iacute; de servicio ya se encuentra deshabilitada.');
                    redirect('categorias-servicio/listar');
                }
            }else{
                $this->session->set_flashdata('warning', 'La categor&iacute; de servicio que intenta deshabilitar no existe.');
                redirect('categorias-servicio/listar');
            }
        }else{
            //Si llegué a este punto es porque no ha ingresado, o no es Administrador
            ...
            if($categoria_servicio){
                if(!$categoria_servicio['habilitado']){
                    ...
                    if($was_updated){
                        ...
                        $this->session->set_flashdata('success', 'La categor&iacute;a de servicio fue abilitada satisfactoriamente. Recuerde que al habilitar una categor&iacute;a, tambi&eacute;n est&aacute; habilitando los servicios pertenencientes a la misma.');
                        redirect('categorias-servicio/listar');
                    }else{
                        $this->session->set_flashdata('danger', 'No se pudo habilitar la categor&iacute; de servicio, por favor intente nuevamente.');
                        redirect('categorias-servicio/listar');
                    }
                }else{
                    $this->session->set_flashdata('warning', 'La categor&iacute; de servicio ya se encuentra habilitada.');
                    redirect('categorias-servicio/listar');
                }
            }else{
                $this->session->set_flashdata('warning', 'La categor&iacute; de servicio que intenta habilitar no existe.');
                redirect('categorias-servicio/listar');
            }
        }else{
            //Si llegué a este punto es porque no ha ingresado, o no es Administrador
