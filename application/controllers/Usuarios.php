...
class Usuarios extends CI_Controller {
    ...
    function registro(){
        ...
        if(!$this->form_validation->run()){
            ...
            if($was_inserted){
                $this->session->set_flashdata('success', 'El usuario fue creado satisfactoriamente.');
                redirect('inicio');
            }

            //Si llegué a este punto es porque no pudo guardar el usuario
            $this->session->set_flashdata('danger', 'No se pudo crear el usuario, por favor intente de nuevo.');
            redirect('inicio');
        }
    }
    ...
    public function detalles($cedula){
        ...
        if($administrador || ($cedula === $cedula_sesion)){
            ...
            if($usuario){
                ...
                $this->session->set_flashdata('danger', 'El usuario que intenta visualizar no existe, o hubo un problema al conectarse con la Base de Datos. Por favor intente nuevamente.');
                redirect('inicio');
            }
        }else{
            //Si llegué a este punto es porque no ha ingresado, o no es Administrador
    ...
    public function actualizar($id){
        ...
        if($administrador || ($id === $id_sesion)){
            ...
            if($usuario){
                ...
                $this->session->set_flashdata('danger', 'El usuario que intenta actualizar no existe o hubo un problema al conectarse con la Base de Datos. Por favor intente nuevamente.');
                redirect('inicio');
            }
            ...
            if (!$this->form_validation->run()){
                ...
                //Si lo guardó correctamente, redirigir al inicio con éxito
                if($was_updated){
                    $this->session->set_flashdata('success', 'El usuario fue actualizado satisfactoriamente.');
                    redirect('usuarios/listar');
                }

                //Si llegué a este punto es porque no pudo guardar el usuario
                $this->session->set_flashdata('danger', 'No se pudo actualizar el usuario, por favor intente de nuevo.');
    ...
    public function eliminar($id){
        ...
        if($administrador){
            ...
            if($cantidad_solicitudes > 0){
                $this->session->set_flashdata('warning', 'Este usuario no puede ser eliminado, posee ' . $cantidad_solicitudes . ' solicitudes. Elimine las solicitudes primero, o deshabilite el usuario en lugar de eliminarlo.');
                redirect('usuarios/listar');
            }else{
                $usuario = $this->usuarios_model->get_usuario_by_id($id);
                if($usuario){
                    ...
                    if($delete_id){
                        $this->session->set_flashdata('success', 'Usuario eliminado satisfactoriamente.');
                        redirect('usuarios/listar');
                    }else{
                        $this->session->set_flashdata('danger', 'No se pudo eliminar su usuario, por favor intente nuevamente');
                        redirect('usuarios/listar');
                    }
                }else{
                    $this->session->set_flashdata('warning', 'El usuario que intenta eliminar no existe.');
                    redirect('usuarios/listar');
                }
            }
        }else{
        ...
        if($administrador){
            ...
            if($usuario){
                if($usuario['habilitado']){
                    ...
                    if($was_updated){
                        $this->session->set_flashdata('success', 'El usuario fue deshabilitado satisfactoriamente. Recuerde que al deshabilitar un usuario, tambi&eacute;n est&aacute; deshabilitando sus solicitudes.');
                        redirect('usuarios/listar');
                    }else{
                        $this->session->set_flashdata('danger', 'No se pudo deshabilitar el usuario, por favor intente nuevamente.');
                        redirect('usuarios/listar');
                    }
                }else{
                    $this->session->set_flashdata('warning', 'El usuario ya se encuentra deshabilitado.');
                    redirect('usuarios/listar');
                }
            }else{
                $this->session->set_flashdata('warning', 'El usuario que intenta deshabilitar no existe.');
                redirect('usuarios/listar');
            }
        }else{
            //Si llegué a este punto es porque no ha ingresado, o no es Administrador
        ...
        if($administrador){
            ...
            if($usuario){
                if(!$usuario['habilitado']){
                    ...
                    if($was_updated){
                        $this->session->set_flashdata('success', 'El usuario fue habilitado satisfactoriamente. Recuerde que al habilitar un usuario, tambi&eacute;n est&aacute; habilitando sus solicitudes.');
                        redirect('usuarios/listar');
                    }else{
                        $this->session->set_flashdata('danger', 'No se pudo habilitar el usuario, por favor intente nuevamente.');
                        redirect('usuarios/listar');
                    }
                }else{
                    $this->session->set_flashdata('warning', 'El usuario ya se encuentra habilitado.');
                    redirect('usuarios/listar');
                }
            }else{
                $this->session->set_flashdata('warning', 'El usuario que intenta habilitar no existe.');
                redirect('usuarios/listar');
            }
        }else{
            //Si llegué a este punto es porque no ha ingresado, o no es Administrador
